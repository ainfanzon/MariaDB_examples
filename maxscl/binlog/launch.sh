function launch_demo(){
  ...
  for s in {mdbsrv1,mdbsrv2,mdbsrv3,mdbsrv4,mdbmxscl};
    do
      case ${s} in
        ...
        ;;
        mdbsrv4) docker exec -i ${s} cp ${TGT}/server.cnf.srv4 /etc/my.cnf.d/server.cnf
                 docker exec -i ${s} /bin/sh -c 'systemctl start mariadb'
                 sleep 5
                 docker exec -i ${s} /bin/sh -c 'mariadb < /mdb/scripts/strt_rep.sql'
                 ${HOME}/scripts/strtTerm.osa "docker exec -it ${s}  mariadb -u mdb -pletmein -h mdbsrv4"
        ;;
        mdbmxscl) docker exec -i mdbmxscl cp ${TGT}/maxscale.cnf /etc/maxscale.cnf
                  ...
                  ${HOME}/scripts/strtTerm.osa "docker exec -it ${s}  maxctrl"
        ;;
      esac
    done
